...
export default function MyGames({ games, isEditable }: MyGamesProps) {
  ...
  // Fetch full game details for each game
  const { data: gameDetails } = useQuery({
    queryKey: ['game-details', games.map(g => g.id)],
    queryFn: async () => {
      if (import.meta.env.MODE === 'development') {
        const details = await Promise.all(
          games.map(game => getDemoGame(game.id))
        );
    ...
    <div className="bg-gaming-card rounded-lg border border-gaming-neon/20">
      <div className="p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {games.map((game) => {
            const gameDetail = gameDetails?.find(d => d.id === game.id);
            return (
              <div
                key={game.id}
              ...
              >
                <img
                  src={gameDetail?.wallPhoto || `https://images.unsplash.com/photo-1542751371-adc38448a05e?auto=format&fit=crop&q=80&w=100&h=100`}
                  alt={game.name}
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between">
                    <div>
                      ...
                      <span className="text-sm text-gaming-neon">{gameDetail?.gameType}</span>
                    </div>
                    {isEditable && (
